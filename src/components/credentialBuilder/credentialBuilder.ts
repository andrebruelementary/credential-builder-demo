...
export class CredentialBuilder {
  ...
  private dynamicPropertiesContainer: HTMLElement;
  private loadTemplateBtn: HTMLButtonElement;
  private saveTemplateBtn: HTMLButtonElement;
  private addPropertyBtn: HTMLButtonElement;
  private issueCredentialBtn: HTMLButtonElement;
  ...
  private async openTemplateSelector(): Promise<void> {
    // We'll implement this in templateSelector component
    // For now, just load a sample template for demonstration
    ...
    // this.loadTemplate(sampleTemplate);
  ...
  private async saveTemplate(): Promise<void> {
    ...
    // DO NOT add the subject as a property - it's handled separately
    // Remove or comment out this code:
    /*
    properties.push({
      id: 'subject',
      label: 'Credential subject (title)',
      type: 'text',
      required: true
    });
    */
